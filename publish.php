...
if (!empty($_POST['name'])) {
    ...
    //根据用户名查询用户
    $sql = "SELECT * FROM `m_goods` WHERE `name` = '{$name}' LIMIT 1";
    $obj = mysql_query($sql);
    ...
    //入库处理
    $sql = "INSERT `m_goods`(`name`,`price`,`des`,`content`,`pic`,`user_id`,`created_time`,`updated_time`,`view`) VALUES('{$name}','{$price}','{$des}','{$content}','{$pic}','{$userId}','{$now}','{$now}',0)";
